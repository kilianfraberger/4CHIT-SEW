...
class Program
{
    ...
    static void Main()
    {
        ...
        new Thread(HandleInput) { IsBackground = true }.Start();
    ...
    static void InitializeSections()
    {
        for (int i = 0; i < SectionCount; i++)
        {
            ...
            sections[i].SectionChanged += SectionChangedHandler;
        }
    }
    ...
    static void HandleInput()
    {
        while (true)
        {
            if (Console.ReadKey(true).Key == ConsoleKey.Spacebar)
            {
                ...
                Train t = new Train(id, length, sections);
                t.TrainMoved += TrainMovedHandler;
                t.TrainStatusChanged += TrainStatusHandler;

                new Thread(t.Run) { IsBackground = true }.Start();
            }
        }
    }
    ...
    static void DrawSections()
    {
        lock (consoleLock)
        {
            ...
            for (int i = 0; i < SectionCount; i++)
            {
                top.Append('|'.ToString().PadRight(SectionWidth));
                bottom.Append('/'.ToString().PadRight(SectionWidth));
            }

            Console.SetCursorPosition(0, 1);
            Console.Write(top.ToString());
            Console.SetCursorPosition(0, 2);
            Console.Write(bottom.ToString());
        }
    }

    static void TrainMovedHandler(object? sender, TrainMovedEventArgs e)
    {
        lock (consoleLock)
        {
            ...
            int tailPos = e.NewPosition - e.Train.Length;
            if (tailPos >= 0 && tailPos < TrackLength)
                trackLine[tailPos] = '=';
    ...
    static void TrainStatusHandler(object? sender, TrainStatusEventArgs e)
    {
        lock (consoleLock)
        {
    ...
    static void SectionChangedHandler(object? sender, SectionChangedEventArgs e)
    {
        lock (consoleLock)
        {
    ...
    class Train
    {
        ...
        public Train(int id, int length, Section[] sections)
        {
            Id = id;
            Length = length;
